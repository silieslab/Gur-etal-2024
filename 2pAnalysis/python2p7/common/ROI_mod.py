...
class ROI_bg: 
    ...
    def findMaxResponse_all_epochs(self):
        ...
        self.max_resp_all_epochs = \
            np.empty(shape=(int(self.stim_info['epoch_number']),1))
    ...
    def calculate_DSI_PD(self,method='PDND'):
        ...
        def find_opp_epoch(self, current_dir, current_freq, current_epoch_type):
            required_epoch_array = \
                    ...
                    (self.stim_info['stim_type'] == current_epoch_type)  
        ...
        if method == 'PDND':
            ...
            current_epoch_type = self.stim_info['stim_type'][self.max_resp_idx]
    ...
    def calculate_CSI(self, frameRate = None):
        ...
        # Find edge epochs
        edge_epochs = np.where(self.stim_info['stim_type']==50)[0]
...
#        trace = signal.filtfilt(b, a, raw_trace)
    ...
    def calculateTFtuning_BF(self):
        ...
        grating_epochs = np.where(((self.stim_info['stim_type'] == 61) | \
                                   (self.stim_info['stim_type'] == 46)) &\
        ...
        current_epoch_type = self.stim_info['stim_type'][max_grating_epoch]
...
def find_opp_epoch_roi(roi, current_dir, current_freq, current_epoch_type):
            required_epoch_array = \
                    ...
                    (roi.stim_info['stim_type'] == current_epoch_type)  
...
def calculate_edge_timing(rois):
    ...
    for roi in rois:
        # Find edge epochs
        edge_epochs = np.where(roi.stim_info['stim_type'] == 50)[0]
        # Find the edge epoch with max response
...
def make_ROI_tuning_summary(rois_df, roi,cmap='coolwarm', plot_x='reliability',plot_y='CSI'):
    ...
    # Plot edge epochs
    grating_epochs = np.where(((roi.stim_info['stim_type'] == 61) | \
                             (roi.stim_info['stim_type'] == 46))\
    ...
    for idx, freq in enumerate(unique_freqs):
        ...
        current_epoch_type = roi.stim_info['stim_type'][max_epoch]
...
def map_RF(rois,edges=True,screen_dim = 60):
    ...
    for i, roi in enumerate(rois):
        ...
        for epoch_idx, epoch_type in enumerate(roi.stim_info['stim_type']):
            if epoch_type == 50:
...
def calculate_roi_delay(roi):
    ...
    for epoch_idx, epoch_type in enumerate(roi.stim_info['stim_type']):
        if epoch_type == 50:
            ...
            curr_type = roi.stim_info['stim_type'][epoch_idx]
...
def fit_gaussian_RFs(rois):
    for roi in rois:
        rf = roi.RF_map_norm.copy()
        rf = rf[30:80,18:94]
        a = rf.copy()
        a[np.isnan(a)] = 0
        gauss_params, success, fit, r_squared = fitTwoDgaussian(a)
        (height, x, y, width_x, width_y) = gauss_params
        roi.rf_fit = fit
        roi.rf_fit_rsq = r_squared
        roi.fit_width_x = width_x
        roi.fit_width_y = width_y
    return rois

def plot_RFs(rois, number=None, f_w =None,cmap='inferno',
             center_plot = False, center_val = 0.95, fit_plot=False):
    import random
    plt.close('all')
    colors = pac.run_matplotlib_params()
    ...
    for idx, roi in enumerate(rois):
        if idx == number:
            break
        roiRF = roi.RF_map_norm[30:80,18:94]
        center_RF = copy.deepcopy(roiRF)
        center_RF[center_RF<center_val] =np.nan
        ax[idx].imshow(roiRF, cmap=cmap,alpha=.8)
        if fit_plot:
            ax[idx].contour(roi.rf_fit,alpha=.5,cmap=cmap)
...
def fit_1d_gauss(data_x, data_y):
    mean = sum(data_x*data_y)/len(data_x) 
    sigma1 = sum(data_y*(data_x-mean)**2)/len(data_x) 
    p0 = [np.max(data_y), np.argmax(data_y), sigma1]
...
def conc_traces(rois, interpolation = True, int_rate = 10):
    ...
    for roi in rois:
        conc_trace = []
        stim_trace = []
        for idx, epoch in enumerate(range(roi.stim_info['epoch_number'])):
        ...
        if interpolation:
            roi.int_con_trace = interpolate_signal(conc_trace, 
                                                   ...
                                                   int_rate,int(round(len(conc_trace)/roi.imaging_info['frame_rate']))) #TODO: add the time for interpolation
...
def calculate_correlation(rois,stim_type = None):
    ...
    for roi in rois:
        ...
        elif stim_type == 'ContrastSteps':
            resp = roi.contrast_responses[1:]
            stim_to_correlate = roi.epoch_contrasts[1:]
...
def analyze_luminance_edges(rois,int_rate = 10):
    ...
    epoch_types = rois[0].stim_info['stim_type']
    epoch_file_l = np.array(rois[0].stim_info['input_data']['Stimulus.lum'],float)
    epoch_file_c = np.array(rois[0].stim_info['input_data']['Stimulus.contrast'],float)
    ...
    for roi in rois:
        ...
        int_len = len(interpolate_signal(ex_trace, 
                                     roi.imaging_info['frame_rate'],int_rate,roi.stim_info['epochs_duration'][np.where(req_epochs)[0][0]]))
        ...
        int_len_wholeT = len(interpolate_signal(ex_trace_wholeT, 
                                     roi.imaging_info['frame_rate'],int_rate))#TODO: add the time for whole traces
        ...
        for idx,  take in enumerate(req_epochs):
            if take:
                ...
                int_traces[mat_idx,:] = interpolate_signal(curr_trace, 
                                     roi.imaging_info['frame_rate'],int_rate,roi.stim_info['epochs_duration'][idx])
...
def analyze_contrast_flashes(rois,int_rate=10):
    ...
    epoch_contrasts = \
        (epoch_luminances -epoch_lum_BG)/epoch_lum_BG

    for roi in rois:
        roi.epoch_lum_BG = epoch_lum_BG
        roi.epoch_luminances = epoch_luminances
        roi.epoch_contrasts = epoch_contrasts
        ...
        int_len = len(interpolate_signal(ex_trace, 
                                     roi.imaging_info['frame_rate'],int_rate,round(len(ex_trace)/roi.imaging_info['frame_rate']))) 
        int_traces = np.zeros((len(roi.whole_trace_all_epochs),int_len))
        mat_idx = 0
        
        roi.contrast_responses = np.full(np.shape(epoch_contrasts),np.nan)
        ...
        for epoch in roi.whole_trace_all_epochs:
            ...
            roi.contrast_responses[epoch] = np.max(resp_t) - base_activity
            traces[mat_idx,:] = whole_t
            
            int_traces[mat_idx,:] = interpolate_signal(whole_t, 
                                     roi.imaging_info['frame_rate'],int_rate,round(len(whole_t)/roi.imaging_info['frame_rate'])) 
            mat_idx +=1
            
        roi.int_rate = int_rate
        roi.resp_traces = traces
        roi.resp_traces_interpolated = int_traces
        roi.resp_max = traces.max()
...
def analyze_A_B_step(rois,int_rate = 10):
    ...
    for roi in rois:
        ...
        ex_trace = roi.whole_trace_all_epochs[1][:min_len]
        int_len = len(interpolate_signal(ex_trace, 
                                     roi.imaging_info['frame_rate'],int_rate)) #TODO: add the time for interpolation
        int_traces =np.zeros((len(roi.whole_trace_all_epochs),int_len))
        mat_idx = 0
...
def analyze_luminance_steps(rois,int_rate = 10):
    ...
    for roi in rois:
        ...
        int_len = len(interpolate_signal(ex_trace, 
                                     roi.imaging_info['frame_rate'],int_rate,int(roi.stim_info['epochs_duration'][1])))
        ...
        for epoch in roi.resp_trace_all_epochs:
            ...
            int_traces[mat_idx,:] = interpolate_signal(curr_trace, 
                                     roi.imaging_info['frame_rate'],int_rate,int(roi.stim_info['epochs_duration'][epoch]))
            mat_idx +=1
...
def analyze_luminance_gratings(rois):
    ...
    epoch_dirs_no_base= \
        np.delete(epoch_dirs,rois[0].stim_info['baseline_epoch'])
    epoch_types = rois[0].stim_info['stim_type']
    epoch_luminances= np.array(rois[0].stim_info['input_data']['Stimulus.lum'],float)
...
def analyze_luminance_gratings_t45(rois):
    epoch_dirs = rois[0].stim_info['epoch_dir']
    epoch_dirs_no_base= \
        np.delete(epoch_dirs,rois[0].stim_info['baseline_epoch'])
    epoch_types = rois[0].stim_info['stim_type']
    ...
    for roi in rois:
        ...
        roi.luminances = np.array(roi.stim_info['input_data']['Stimulus.lum'],float)[req_epochs]  
        roi.frequencies =   roi.stim_info['epoch_frequency'][req_epochs]  
        ...
        for idx,epoch in enumerate(np.where(req_epochs)[0]):
            ...
            req_idx = np.argmin(np.abs((xf[1:N//2])-curr_freq)) # TODO: not good for proper selection of slow frequencies
...
def analyzeSineGratings(rois,int_rate = 10):
    ...
    for roi in rois:
        ...
        for epoch in used_epochs:
            ...
            # Interpolate traces 
            roi.int_resp_trace[epoch] = interpolate_signal(roi.resp_traces[epoch],roi.imaging_info['frame_rate'],int_rate=int_rate,) 
...
def analyze_luminance_gratings_1Hz(rois,int_rate = 10):
    ...
    for roi in rois:
        ...
        ex_trace = roi.resp_trace_all_epochs[np.where(req_epochs)[0][0]][:min_len]
        int_len = len(interpolate_signal(ex_trace, roi.imaging_info['frame_rate'],int_rate,roi.stim_info['epoch_dur'][np.where(req_epochs)[0][0]]))
        int_traces =np.zeros((np.sum(req_epochs),int_len))
        ...
        ex_trace_wholeT = roi.whole_trace_all_epochs[np.where(req_epochs)[0][0]][:min_len_wholeT]
        int_len_wholeT = len(interpolate_signal(ex_trace_wholeT, 
                                     roi.imaging_info['frame_rate'],int_rate,int(round(len(ex_trace_wholeT)/fr)))) 
        int_traces_wholeT =np.zeros((np.sum(req_epochs),int_len_wholeT))
        mat_idx = 0
        for idx,epoch in enumerate(np.where(req_epochs)[0]):
            curr_freq = roi.stim_info['epoch_frequency'][epoch]
            ...
            traces[mat_idx,:] = curr_trace
            int_traces[mat_idx,:] = interpolate_signal(curr_trace, 
                                 roi.imaging_info['frame_rate'],int_rate,int(round(len(curr_trace)/fr))) #TODO: time for interpolation
            
            curr_trace_wt = roi.whole_trace_all_epochs[epoch][:min_len_wholeT]
            traces_wholeT[mat_idx,:] = curr_trace_wt
            int_traces_wholeT[mat_idx,:] = interpolate_signal(curr_trace_wt, 
                                 roi.imaging_info['frame_rate'],int_rate,int(round(len(curr_trace_wt)/fr))) #TODO: time for interpolation
        ...
        roi.slope = linregress(X, np.transpose(roi.power_at_hz))[0]
        roi.basePower_slope = linregress(X, np.transpose(Z))[0]
        roi.base_slope = linregress(X, np.transpose(roi.baselines))[0]
        
        curr_coeff, pval = spearmanr(roi.power_at_hz,roi.luminances)
...
def analyze_lum_con_gratings(rois):
    ...
    epoch_luminances = np.array(rois[0].stim_info['input_data']['Stimulus.lum'],float)
    epoch_contrasts = np.array(rois[0].stim_info['input_data']['Stimulus.contrast'],float)
...
def analyze_gratings_general(rois,int_rate = 10):
    """ Analysis for sinusoidal gratings """
    ...
    for roi in rois:
        ...
        roi.luminances = epoch_luminances[req_epochs]
        roi.contrasts = epoch_contrasts[req_epochs]            
        conc_trace = []
        roi.power_at_hz = np.zeros_like(roi.luminances)
        roi.base_power = np.zeros_like(roi.luminances)
        roi.baselines = np.zeros_like(roi.luminances)
