...
#%% Following analyzes 5s full field flashes for Tm1 iGluSnFR recordings.
#%%
import cPickle
import os
import numpy as np
...
for idataset, dataset in enumerate(datasets_to_load):
    ...
    if (not('FFF-Dark5sBright5s' in curr_rois[0].stim_info['meta']['stim_name'])):
        continue
    ...
    curr_rois = PyROI.fffAnalyze(curr_rois,int_rate=10)
...
ax.plot(t_trace,
        plot_stim/9+ fff_all_max,'--k',lw=1.5,alpha=.8)
